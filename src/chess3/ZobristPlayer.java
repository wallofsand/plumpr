...
public class ZobristPlayer extends Player {
	...
	public long perft(int depth) {
		if (depth == 0)
			return 1;
		MoveGenerator perftgen = new MoveGenerator(ch);
		// Why did having this code break the perft() method?
//		if (perftgen.moves.size() == 0)
//			return 1;
		int i = 0;
...
//		Zobrist.sethits(0);
	...
	float negaMax(int depth, float alpha, float beta) throws InterruptedException {
		...
		// hash the position
		long zhash = Zobrist.hash(test);
		// if the game has ended, eval
		// if the search has reached target depth, begin quiescence search
...
//					score += (PieceLocationTables.read(tables[Piece.type(test.b0[sq])], sq, true)) / 100f;
					
				} else {
					materialScore -= pieceValue[Piece.type(piece)];
					positionalScore -= PieceLocationTables.complexRead(Piece.type(piece), sq, midgameWeight, false);
//					score -= (PieceLocationTables.read(tables[Piece.type(test.b0[sq])], sq, false)) / 100f;
				}
			}
		}
